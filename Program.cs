...
internal interface IVehicle
{
    ...
    internal decimal CalculateRent(int Units); internal void getDetails();
    internal DateOnly getLastMaintenanceDate();
}
internal class Indica : IVehicle
{
    internal string? type, number;
    internal IVehicle.Renttype renttype; internal int age, rentperunit, seater; internal DateOnly last_maintenance_date;
    internal Indica(string type, int seater, string number, IVehicle.Renttype rentType, int age, int rentperunit, DateOnly last_maintenance_date)
    {
        ...
        renttype = rentType;
        this.age = age;
        this.rentperunit = rentperunit; this.last_maintenance_date = last_maintenance_date;
    }
...
internal class MercedesBenz : IVehicle
{
    ...
    {
        this.type = type; this.seater = seater; this.number = number;
...
{
    internal T carobj;
    internal DateOnly startDate, endDate; internal int Units;
    internal decimal advPayment;
    internal CarType(T carobj, DateOnly startDate, DateOnly endDate, decimal advPayment)
    {
        this.carobj = carobj; this.advPayment = advPayment; this.startDate = startDate; this.endDate = endDate;
    }
    ...
    internal int CalculateDays()
    {
        ...
        int month = endDate.Month - startDate.Month; int day = endDate.Day - startDate.Day;
        return year + month + day;
    }
}
internal class RentedVehicle<T>
{
    List<CarType<T>> Vehiclelist; internal RentedVehicle()
    {
    ...
    {
        Console.Write($"\n\n Available Vehicles on {date}"); foreach (CarType<T> c in Vehiclelist)
        {
...
class Program
{
    static void Main(string[] args)
    {
        ...
        Indica i1 = new Indica("Petrol", 5, "GJ-01-XY-1021", IVehicle.Renttype.Day, 10, 13, new DateOnly(2019, 12, 01));
        MercedesBenz mb1 = new MercedesBenz("Diesel", 7, "GJ- 01-AB-7875", IVehicle.Renttype.KM, 3, 17, new DateOnly(2020, 02, 11));
        Qualis q1 = new Qualis("Diesel", 7, "GJ-04-GS-5700", IVehicle.Renttype.KM, 7, 5, new DateOnly(2021, 09, 25));
        Qualis q2 = new Qualis("CNG", 4, "GJ-01-HI-0210", IVehicle.Renttype.KM, 15, 14, new DateOnly(2022, 02, 28));

        MercedesBenz mb2 = new MercedesBenz("Petrol", 7, "GJ- 01-AB-7875", IVehicle.Renttype.KM, 3, 17, new DateOnly(2021, 02, 12));
        RentedVehicle<IVehicle> rv = new RentedVehicle<IVehicle>();
        rv.AddVehicle(i1); rv.AddVehicle(mb1); rv.AddVehicle(q1); rv.AddVehicle(q2); rv.AddVehicle(mb2);
